...
# Проверяем ApiResponses.kt
echo "1. Проверяем ApiResponses.kt:"
...
# Проверяем импорты в AuthRoutes.kt
echo "2. Проверяем импорты в AuthRoutes.kt:"
head -15 src/main/kotlin/com/trusttheroute/backend/routes/auth/AuthRoutes.kt | grep -E "import|ErrorResponse|MessageResponse"

echo ""
echo "3. Используем явный импорт ErrorResponse и MessageResponse..."

# Обновляем AuthRoutes.kt с явным импортом
...
echo "✅ Файл обновлен с явными импортами"
echo ""
echo "Пересобираем..."
./gradlew clean build
